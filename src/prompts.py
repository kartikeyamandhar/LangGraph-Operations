...
REPORT_PROMPT = ChatPromptTemplate.from_messages([
    ("system",
     ...
     "RULE 2 — Banner (the FIRST line of the report; pick exactly one):\n"
     "  ⚠️ OUTPUT DISCIPLINE: decide the banner SILENTLY. The banner is ONE\n"
     "  short sentence. NEVER print your reasoning, the field names, the words\n"
     "  'f1'/'f2'/'yes'/'no', question marks, or any decision steps. Output\n"
     "  ONLY the final banner sentence (with its emoji). Anything else is a bug.\n\n"
     "  Evaluate these conditions TOP-TO-BOTTOM and use the FIRST that is true:\n"
     "    1. `audit_feedback` starts with 'UNRESOLVED VIOLATIONS' →\n"
     "       🔴 Plan force-passed with unresolved violations after 3 retries — see Section 8.\n"
     "    2. `human_approval_required`=True and `human_approved`=False →\n"
     "       🔴 Critical corridor risk · Manager approval NOT granted — plan cannot proceed.\n"
     "    3. `deferral_summary.tier1_units_deferred` > 0 →\n"
     "       🔴 Tier-1 SLA at risk · <tier1_units_deferred> life-critical units deferred.\n"
     "    4. `deferral_summary.tier2_units_deferred` > 0 (and Tier-1 is 0) →\n"
     "       🟠 <deferred_units> units deferred under capacity limits · review allocation.\n"
     "    5. `human_approval_required`=True and `human_approved`=True →\n"
     "       🟠 Critical corridor risk · Plan approved by manager.\n"
     "    6. any corridor route_risk_score_0_3 == 2 →\n"
     "       🟡 Elevated weather risk · 25% buffer applied · plan within tolerance.\n"
     "    7. otherwise →\n"
     "       🟢 All clear · plan within nominal parameters.\n\n"
     "  HARD CONSISTENCY RULE (this is where mistakes happen — get it right):\n"
     "    • Use 🟢 green ONLY when total_penalty_score == 0 AND deferred_units == 0\n"
     "      AND human_approval_required is False AND audit did not force-pass\n"
     "      AND no corridor risk == 2. If any of those is violated, 🟢 is WRONG.\n"
     "    • Use case 3 (🔴 Tier-1) ONLY when tier1_units_deferred > 0. If it is 0,\n"
     "      you must NOT write 'Tier-1 SLA at risk' — that would be a false alarm.\n"
     "    • APPROVAL DISAMBIGUATION (cases 2 vs 5): when human_approval_required\n"
     "      is True, read `human_approved` LITERALLY. human_approved == True means\n"
     "      the manager ALREADY approved → case 5 (🟠 'Plan approved by manager').\n"
     "      human_approved == False means NOT approved → case 2 (🔴 'cannot\n"
     "      proceed'). Never show 'cannot proceed' when human_approved is True.\n"
     "    • The banner's number must equal the matching field exactly.\n\n"
