...
REPORT_PROMPT = ChatPromptTemplate.from_messages([
    ("system",
     ...
     "RULE 2 — Banner state machine (pick EXACTLY ONE, top of report):\n"
     "  • If `audit_feedback` starts with 'UNRESOLVED VIOLATIONS' → "
     "    🔴 'Plan force-passed with unresolved violations after 3 retries — see Section 8.'\n"
     "  • Else if `human_approval_required` is True and `human_approved` is False → "
     "    🔴 'Critical corridor risk · Manager approval pending — plan cannot proceed.'\n"
     "  • Else if `allocation_plan.deferral_summary.tier1_units_deferred > 0` → "
     "    🔴 'Tier-1 SLA at risk · X life-critical units deferred.'\n"
     "  • Else if `human_approval_required` is True and `human_approved` is True → "
     "    🟠 'Critical corridor risk · Plan approved by manager.'\n"
     "  • Else if any corridor has route_risk_score_0_3 == 2 → "
     "    🟡 'Elevated weather risk · 25% buffer applied · plan within tolerance.'\n"
     "  • Else → 🟢 'All clear · plan within nominal parameters.'\n"
     "  Use that exact phrasing (with X substituted as needed). No extra words.\n\n"

     "RULE 3 — Tier-1 protection language:\n"
     "  • NEVER claim Tier 1 is 'fully covered', 'protected', 'safeguarded', or "
     "    'within SLA' UNLESS `allocation_plan.deferral_summary.tier1_protected` is True.\n"
     "  • If tier1_protected is False, the executive summary's first bullet MUST be "
     "    exactly: 'X Tier-1 life-critical units deferred — SLA at risk on [corridor list].'\n\n"
